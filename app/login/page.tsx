...
export default function LoginPage() {
  ...
  const submit=async(e:React.FormEvent)=>{
    ...
    try{const{data,error:err}=await supabase.auth.signInWithPassword({email,password});if(err){setError(err.message);return;}if(data.session)router.push('/dashboard');}
    ...
    <div className="neo-auth-root">
      <div className="neo-auth-card fade-up">
        ...
        <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:12}}>
          {[{l:'EMAIL',t:'email',ph:'you@example.com',v:email,s:setEmail},{l:'PASSWORD',t:'password',ph:'••••••••',v:password,s:setPassword}].map((f,i)=>(
            <div key={i}>
              ...
              <input type={f.t} placeholder={f.ph} value={f.v} required onChange={e=>f.s(e.target.value)} className="neu-input"/>
            </div>
          ))}
          <button type="submit" disabled={loading} className="neu-btn neu-btn-black" style={{marginTop:6,justifyContent:'center',width:'100%',padding:13,fontSize:14,opacity:loading?.6:1}}>
