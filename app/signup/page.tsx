...
export default function SignupPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        // Real table is 'profiles', column 'username' (not full_name), 'role' (not is_admin)
  ...
  return (
    <>
      ...
      <main style={{ minHeight: 'calc(100vh - 52px)', background: 'var(--black)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ width: '100%', maxWidth: 380, border: '1.5px solid var(--dim)', animation: 'fadeUp 0.3s ease both' }}>
          ...
          <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                { label: 'Name', type: 'text', ph: 'John Doe', val: fullName, set: setFullName, req: false },
                ...
                { label: 'Confirm', type: 'password', ph: '••••••••', val: confirm, set: setConfirm, req: true },
              ].map((f, i) => (
                <div key={i}>
