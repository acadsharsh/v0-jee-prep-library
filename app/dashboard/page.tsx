  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Real join: quizzes(title) — table is quizzes not quiz
          ...
          <div style={{ display: 'grid', gridTemplateColumns: chartData.length ? '1.5fr 1fr' : '1fr', gap: 0, border: '1.5px solid var(--dim)' }}>
            {chartData.length > 0 && (
              <div style={{ borderRight: '1.5px solid var(--dim)', padding: '24px' }}>
                <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted)', marginBottom: 20 }}>Score by quiz</div>
            ...
            <div style={{ padding: '24px' }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted)', marginBottom: 20 }}>Recent attempts</div>
              {attempts.length ? (
                <div>
                  {attempts.slice(0, 6).map((a, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 0', borderBottom: i < 5 ? '1px solid var(--dim)' : 'none' }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--white)' }}>{a.title.length > 26 ? a.title.slice(0, 26) + '…' : a.title}</div>
                        <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>{a.date}</div>
                      </div>
                      <span style={{ fontFamily: 'Space Mono, monospace', fontSize: 18, fontWeight: 700, color: cssScore(a.score) }}>{a.score}%</span>
                    </div>
                  ))}
                </div>
              ) : (
